...
- **Project rules win over P3C on conflict.** If the project has its own coding-standards doc (`.qoder/docs/coding-standards.md`, `CONTRIBUTING.md`, `docs/style.md`, etc.), follow the project's version and call out the conflict in review output so the reader can decide.
- **crm-java-user project conventions:** This repo's readability rules live in `docs/coding-standards.md` and are enforced by the `java-code-style` skill. Always invoke `java-code-style` in parallel when working in crm-java-user — it covers blank-line structure, cross-service call comments, and private-method extraction that P3C does not address.
